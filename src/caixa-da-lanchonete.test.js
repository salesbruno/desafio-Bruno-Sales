...
describe("CaixaDaLanchonete", () => {
  const validaTeste = (formaDePagamento, resultadoEsperado, itens) => {
    ...
    expect(resultado.replace("\xa0", " ").replace(".", ",")).toEqual(
      resultadoEsperado
    );
  };
  ...
  if (
    ('compra ""com carrinho vazio" em "dinheiro" deve resultar em "Não há itens no carrinho de compra!"',
    () => {
      const caixa = new CaixaDaLanchonete();
      const resultado = caixa.calcularValorDaCompra("dinheiro", ["chantily,1"]);
  ...
  if (
    ('compra "chantily" em "dinheiro" deve resultar em "Item extra não pode ser pedido sem o principal"',
    () => {
      const caixa = new CaixaDaLanchonete();
      const resultado = caixa.calcularValorDaCompra("dinheiro", ["chantily,1"]);
      const resultadoEsperado =
